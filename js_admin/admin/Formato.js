...
$(document).ready(function(){    
    ...
    jqGridPesos = jQuery("#tablaPesos").jqGrid({
        ...
        multiselect: false,        
        //editurl: "../controller/Controller_1.php?clase=cmPeso",
        onSelectRow:function(id){            
            var aux=jQuery(this).jqGrid('getRowData',id);
            annioo=aux.anno;
            ver=aux.version;
            coda=aux.codar;  
            clear(annioo,ver,coda);
...
<div id='pTablaPesosTipo'></div>");    
    ...
    $.ajax({
        type:'POST', 
        url:$("#base_url").val()+'admin/pesocompetencia/selectBox', 
        data:{           
            an:anio,
            vr:vers,
            cda:codar
        ...
        success: function(response) {
            ...
            jqGridPesosTipo = jQuery("#tablaPesosTipo").jqGrid({
                url:$("#base_url").val()+'admin/formato/select', 
                datatype: "json",
                mtype: 'POST',
                postData: {                    
                    anno:anio,
                ...
                colNames:['COMPETENCIA','PESO','CODCOMPETENCIA','ANNO','VERSION','CODAREA','REQUERIDO','CRITICIDAD'],
                colModel:[
                {
                    name:'tipo',
                    index:'tipo',
                ...
                loadonce:true,
                sortorder: "asc",
                caption:"Pesos segun competencia",
                multiselect: false,        
                editurl: $("#base_url").val()+'admin/formato/editData', 
            ...
            {   //Estas sentencias se ejecutan al adicionar
                ...
                beforeSubmit: function(postdata, formid) {
                    if($("#tipo").val()==-1)
                        return [false, "No existe mas competencias", ""];                    
                    postdata['anno']=anio;                    
                    postdata['version']=vers;
                    postdata['codarea']=codar;
                    return [true, "", ""]; // <-- return [éxito, mensaje];
                },
                afterSubmit: function(response, postdata) {                    
                    var respuesta = jQuery.parseJSON(response.responseText);                   
